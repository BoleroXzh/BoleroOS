...
static char* number(char* str,
    ...
    if (num == 0)  //如果要打印的数字是0，直接把0写入tmp就可以了
        ...
        while (num !=
               0)  //否则不停的对num进行除法运算，索引digits字符串。写入tmp
...
int vsprintf(char* buf,
             const char* fmt,
             va_list args)  // fmt是格式说明符号，buf是待写入数据的buffer
{
    int len;
    int i;
    char* str;
    ...
    for (str = buf; *fmt; ++fmt) {  //扫描格式说明符
        if (*fmt !=
            '%') {  //如果没有格式说明符，说明是普通的字符，直接写入str指向的buffer即可
            *str++ = *fmt;
            continue;
        }
        ...
        switch (*fmt) {
            case 'c':                 //如果格式说明符是个c,把此时的参数当作ASCII码对待,说明将参数以字符的形式打印
                if (!(flags & LEFT))  //没有左对齐进入if
                    while (--field_width > 0)
                        *str++ = ' ';
                *str++ = (unsigned char)va_arg(
                    args, int);  //把对应参数写入str指定的buffer
                while (--field_width >
                       0)  //对格式输出宽度有要求的时候，输出补齐空格
                    *str++ = ' ';
                break;
            ...
            case 's':  //说明是个字符串
                ...
                for (
                    i = 0; i < len;
                    ++i)  //这里把s指向的字符串的字符一个个的copy到str指向的buffer
                    *str++ = *s++;
                while (len < field_width--)
                    *str++ = ' ';
                break;
