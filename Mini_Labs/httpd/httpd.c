...
int parse_cmd(int argc, char* argv[], char* root_dir, int* port) {
    if (argc < 3 || argc > 4)
        return 1;
...
void server(int servfd, char* filename, int file_size) {
    int src_fd;
    ...
    Close(src_fd);
    Rio_writen(servfd, src, file_size);
    Munmap(src, file_size);
}

void parse(int servfd, char* root_dir) {
    rio_t rio;
    struct stat sbuf;
    ...
    if (stat(filename, &sbuf) < 0) {
        printf("404: Cannot find this file.\n");
        return;
    }
    ...
    server(servfd, filename, sbuf.st_size);
}
...
int main(int argc, char* argv[]) {
    ...
    int port = 8080;
    int mode = parse_cmd(argc, argv, root_dir, &port);
    if (mode != 0) {
        printf("You should use <./httpd --port [port] [dir]> to run this server\n");
        return 0;
    }
    ...
    while (1) {
        ...
        servfd = Accept(listen_fd, (SA*)&client_addr, (socklen_t*)&client_len);
        parse(servfd, root_dir);
        Close(servfd);
    }
    return 0;
}